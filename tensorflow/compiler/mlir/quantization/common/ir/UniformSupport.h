...
class UniformQuantizedValueConverter {
 public:
  ...
  virtual APInt quantizeFloatToInt(APFloat expressed_value) const {
    ...
    // fixedpoint = clamp(clamp_min, clamp_max, (
    ...
    APFloat fixedpoint = llvm::minimum(scaled, clamp_max_);
    fixedpoint = llvm::maximum(fixedpoint, clamp_min_);

    llvm::APSInt result(storage_bit_width_, !is_signed_);
    fixedpoint.convertToInteger(result, round_mode_, &lossy);
 ...
 private:
  ...
  // arithmetic.
  virtual APInt quantizeF32ToInt8(APFloat expressed_value) const {
